                    ...
                    child: Column(
                      ...
                      children: [
                        _DashboardHeader(
                          ...
                          email: user.email,
                          onLogout: _logout,
                          unreadCountStream: _notificationService
            ...
            children: [
              ...
              StreamBuilder<int>(
                ...
                builder: (context, snapshot) {
                  ...
                  return _NotificationBell(count: count);
                },
              ),
...
class _MonthlyBudgetSection extends StatelessWidget {
  const _MonthlyBudgetSection({required this.uid, required this.budgetService});
  ...
  @override
  Widget build(BuildContext context) {
    final monthKey = BudgetService.currentMonthKey();

    return StreamBuilder<BudgetModel?>(
      stream: budgetService.watchCurrentMonthBudget(uid),
      builder: (context, budgetSnapshot) {
        ...
        return StreamBuilder<double>(
          stream: budgetService.watchMonthlySpent(uid, monthKey),
          builder: (context, spentSnapshot) {
            final spent = spentSnapshot.data ?? 0;
            final budget = budgetSnapshot.data?.copyWithSpent(spent);

            if (budget == null) {
              return _BudgetPromptCard(
                title: 'Set your monthly budget',
                message: 'Spent this month: ${spent.toStringAsFixed(2)}',
                onTap: () => Navigator.pushNamed(context, AppRoutes.budget),
              );
            }
...
class _DashboardBudgetCard extends StatelessWidget {
  const _DashboardBudgetCard({required this.budget, required this.onTap});
  ...
  @override
  Widget build(BuildContext context) {
    final color = budget.status == BudgetStatus.over
        ? AppColors.red
        : budget.status == BudgetStatus.near
        ? Colors.orange
        : AppColors.success;
          ...
          children: [
            ...
            Row(
              children: [
                ...
                _BudgetMiniMetric(
                  label: 'Spent',
                  value: budget.spent.toStringAsFixed(2),
                ),
                _BudgetMiniMetric(
                  label: 'Left',
...
class _BudgetPromptCard extends StatelessWidget {
...
class _BudgetMiniMetric extends StatelessWidget {
